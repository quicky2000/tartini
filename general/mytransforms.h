...
{  
  ...
  */
  void init(int p_n, int p_k, double p_rate, bool p_equal_loudness=false, int p_num_harmonics=40);
  ...
  */
  double autocorr(float *p_input, float *p_output);
  double autoLogCorr(float *p_input, float *p_output);
  ...
  /**
     ...
     @param ouput. This should be an array of length k
  */
  double asdf(float *p_input, float *p_output);
  ...
  */
  double nsdf(float *p_input, float *p_output);
  ...
  */
  static int findNSDFMaxima(float *p_input, int p_len, std::vector<int> &p_max_positions);

  /**
     @return The index of the first sub maximum.
     This is now scaled from (threshold * overallMax) to 0.
  */
  static int findNSDFsubMaximum(float *p_input, int p_len, float p_threshold);
  ...
  */
  void calculateAnalysisData(/*float *input, */int p_chunk, Channel *p_channel/*, float threshold*/);

  /**
     Do a Windowed FFT for use in the Active FFT data window
  */
  void doChannelDataFFT(Channel *p_channel, float *p_current_input, int p_chunk);
  void calcHarmonicAmpPhase(float *p_harmonic_amp, float *p_harmonic_phase, int p_bins_per_harmonic);
  void doHarmonicAnalysis(float *p_input, AnalysisData &p_analysis_data, double p_period);
  ...
  */
  ...
  double get_max_note_change(float *p_input, double p_period);
  ...
  */
  static double calcFreqCentroid(float *p_buffer, int p_len);
  ...
  /**
     ...
     @return The normalised frequency centriod
  */
  static double calcFreqCentroidFromLogMagnitudes(float *p_buffer, int p_len);
