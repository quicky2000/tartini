...
static double g_high_pass_filter_coeff_B[6][3] =
  {
    { //96000 Hz
...
static double g_high_pass_filter_coeff_A[6][3] =
  {
    { //96000 Hz
      1.0,
...
QString Channel::getUniqueFilename() const
{
    ...
    {
        return QString::fromStdString(getFilenamePart(getParent()->getFileName())) + l_ending_star;
    }
    else
    {
        int l_index = getParent()->getChannelIndex(*this);
        return QString::fromStdString(getFilenamePart(getParent()->getFileName())) + " (" + QString::number(l_index + 1) + ")" + l_ending_star;
    }
}
