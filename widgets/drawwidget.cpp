...
{
    m_painter.fillRect(0, 0, width(), height(), GData::getUniqueInstance().backgroundColor());
}
    ...
    if (l_base_X > 1)
    {
        ...
        for(; l_n < l_the_width && l_base_element < l_last_base_element; l_n++, l_base_element++)
        {
            ...
            if(l_zoom_element.high() != 0.0f && l_zoom_element.high() - l_zoom_element.low() < 1.0)
            {
                ...
                p_painter.drawLine( l_n
                                  , p_paint_device.height() - m_line_top_half_width - toInt(l_zoom_element.high() / p_zoom_Y) + l_view_bottom_offset
                                  , l_n
                                  , p_paint_device.height() + m_line_bottom_half_width - toInt(l_zoom_element.low() / p_zoom_Y) + l_view_bottom_offset
                                  );
            }
        }
    }
    else
    {
        ...
        for (double l_n = l_start; l_n < l_stop && l_int_chunk < static_cast<int>(p_channel->totalChunks()); l_n += l_step_size, l_int_chunk++)
        {
            ...
            if(GData::getUniqueInstance().pitchContourMode() == 0)
            {
                ...
                {
                    p_painter.setPen(QPen(colorBetween(GData::getUniqueInstance().backgroundColor(), p_channel->get_color(), l_err * vol), m_line_width));
                }
            }
            else
            {
                                  ...
                                  )
{
    ...
    if (l_base_X > 1)
    {
        ...
        p_painter.setBrush(GData::getUniqueInstance().shading1Color());
        p_painter.drawRect(l_first_N, 0, l_last_N, height());
        p_painter.setPen(GData::getUniqueInstance().shading2Color());
        if(l_point_index > 1)
        {
    ...
    else
    {
        ...
        p_painter.setBrush(GData::getUniqueInstance().shading1Color());
        p_painter.drawRect(l_first_N, 0, l_last_N, height());
        p_painter.setBrush(GData::getUniqueInstance().shading2Color());
