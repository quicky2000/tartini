...
class FreqPair
{
  public:
    ...
    void attachNext(FreqPair * p_next)
    {
        m_next = p_next;
    }
};
...
class FreqHistory
{
  public:
    ...
    std::vector<FreqPair> * curData()
    {
        return &m_data[m_end];
    }

    std::vector<FreqPair> * prevData()
    {
        return &m_data[((m_end > 0) ? m_end - 1 : m_num - 1)];
    }
    ...
    void connectToPrev()
    {
        ...
        unsigned int l_prev_pos = 0;
        unsigned int l_cur_pos = 0;
